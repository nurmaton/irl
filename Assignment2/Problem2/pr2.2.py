...
#initialize a map to store the best actions (represented as strings)
policyMap = np.empty((gridSize, gridSize), dtype=str)
policyMap[:] = ' '
...
for it in range(numIterations):
    ...
    for state in states:
        #Compute the Bellman Optimality iterate:
        #V(s) = max_a [ r(s,a) + gamma * V(s_next) ]
        actionValues = []
        for action in actions:
            nextPosition, reward = actionRewardFunction(state, action)
            actionValues.append(reward + gamma * copyValueMap[nextPosition[0], nextPosition[1]])
        
        #take the max for Value Iteration
        valueMap[state[0], state[1]] = np.max(actionValues)

        #find which action gave us that maximum value
        best_action_idx = np.argmax(actionValues)

        #record the optimal action, or mark as 'T' if it's a terminal state
        if state in terminationStates:
            policyMap[state[0], state[1]] = 'T'
        else:
            policyMap[state[0], state[1]] = action_names[best_action_idx]
    
    if not convergenceFlag and np.array_equal(valueMap, copyValueMap):
        convergenceNum = it
        convergenceFlag = True
        

    #for selected iterations, print the value function and the policy
...
print("Convergence Iteration: {}".format(convergenceNum))